  ...
  const rows = places
    ...
    <tr>
      ...
      <td style="padding:12px 16px;">
        <strong style="font-family:'Playfair Display',serif;font-size:1.15rem;">${escapeHtml(p.title)}</strong>
        ${p.address ? `<br/><span style="color:#9ca3af;font-size:0.875rem;">${escapeHtml(p.address)}</span>` : ""}
      </td>
    </tr>`
    )
    .join("");
...
<h1 style="font-family:'Playfair Display',serif;margin-bottom:24px;">Your Saved Places</h1>
<table>${rows}</table>
